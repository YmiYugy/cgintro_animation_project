...
void Boids::updateUBOs()
{
    ...
    char* ptr = static_cast<char *>(glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY));
    for(size_t i = 0; i < instanceCount; i++)
    {
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::rotate(model, glm::radians(i*360.0f/instanceCount), glm::vec3(1.0f, 0.0f, 0.0f));
        glm::mat4 view = glm::mat4(1.0f);
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
        glm::mat4 modelView = view * model;
        memcpy(ptr + i*sizeof(modelView), glm::value_ptr(modelView), sizeof(modelView));
    }
